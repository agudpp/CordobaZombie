...
void DrawablePath::drawPath(const std::vector<core::Vertex> &path, int color)
{
	Ogre::String colorstr;
	switch(color){
	case COLOR_BLUE:
...
Test::Test()
{
	...
	core::Point p1(10,100);
	core::Point p2(100, 10);
...
void Test::loadAditionalData(void)
{
	...
	ASSERT(err == 0);
	std::vector<core::Vertex *> &vv = mVvertexs.getObjs();
