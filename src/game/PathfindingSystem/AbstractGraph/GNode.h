...
class GNode
{
    // we will set Graph as friend class to have access to the mID
    // member (to improve some algorithms)
    friend class Graph;
...
// Get the edges
inline const GEdge *
GNode::getEdge1(void) const
{
    return mEdge1;
}
inline const GEdge *
GNode::getEdge2(void) const
{
    return mEdge2;
}
inline const GEdge *
GNode::getEdge3(void) const
{
    return mEdge3;
}
...
// Set the triangle
inline void
GNode::setTriangle(const Triangle *t)
{
    ASSERT(t);
    mTriangle = t;
}
inline const Triangle*
GNode::getTriangle(void) const
{
    ASSERT(mTriangle);
    return mTriangle;
}
...
// checks if have a neighbor associated
inline bool
GNode::hasNeighbor(const GNode *other) const
{
    return ((mEdge1 && mEdge1->getNeighbor(this) == other)
        || (mEdge2 && mEdge2->getNeighbor(this) == other)
...
// must have at least 3 positions
inline void
GNode::getNeighbors(const GNode **neighbors) const
{
    if (mEdge1) {
...
// REQUIRES: that the other node really share an edge
inline const GEdge *
GNode::getSharedEdge(const GNode *other) const
{
    ASSERT(other);
    ASSERT(hasNeighbor(other));

    if (mEdge1->getNeighbor(this) == other)
        return mEdge1;
    if (mEdge2->getNeighbor(this) == other)
        return mEdge2;
    if (mEdge3->getNeighbor(this) == other)
        return mEdge3;
    return 0;
}
...
// contains V but is not the edge E
inline const sm::Vertex *
GNode::getOppositeVertex(const sm::Vertex *V, const GEdge *R) const
{
