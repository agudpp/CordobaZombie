...
void
Test::loadLevelPathfindLoader(void)
{
    boost::shared_ptr<TiXmlDocument> doc(
            Common::Util::loadXmlDocument("LevelPathfindLoader.xml"));
    ASSERT(doc.get());
    LevelPathfindLoader lpl;
    LoaderData ld;
    ld.pathfindingManager = mLevelManager.getPathfinderManager();
    ASSERT(lpl.load(doc->RootElement(), &ld) == 0);
...
//    mLevelManager.load(ent, 25000, 25000, 100, 100);
    ...
    ASSERT(triNavMesh.build() == 0);

    // export the graph
    TriNavMeshBuilder::exportGraph(triNavMesh.getGraph(), "demo.pfg");
    vertexs.removeAndFreeAll();
    triangles.removeAndFreeAll();
    triNavMesh.destroy();

    loadLevelPathfindLoader();
...
void Test::update()
{
	...
	if(GLOBAL_KEYBOARD->isKeyDown(OIS::KC_P)){
		if(!kPressed5){
			kPressed5 = true;
			const Ogre::Vector3 &pos = mCamController.getCamPos();
			debugGREEN("CamPosition: %f, %f, %f\n", pos.x, pos.y, pos.z);
		}
	} else {
		kPressed5 = false;
	}
