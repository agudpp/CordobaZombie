...
CollisionManager	*Bomb::mCollMngr = 0;
CollisionResult		Bomb::mCollResult;
...
bool Bomb::checkObjectBetween(const math::Vector2 &p1,  const CollisionObject *t)
{
	...
	for(int i = mCollResult.size()-1; i >= 0; --i){
		...
		if(intersecPoints.empty()) continue;
...
Bomb::~Bomb()
{
	...
	GLOBAL_SCN_MNGR->getRootSceneNode()->removeAndDestroyChild(mNode->getName());
 ...
 */
void Bomb::setCollisionManager(CollisionManager *cm)
{
	ASSERT(cm);
	mCollMngr = cm;
}

void Bomb::setEntity(Ogre::Entity *ent)
{
	ASSERT(ent);
	ASSERT(!mEntity);
