...
void Test::createCollectable(void)
{
	...
	wb.configureShoots(100, mLevelManager.getCollisionManager(), &mUpdMngr);
	Weapon *weapon = wb.createWeapon("shotgun");

	////////////
	mCollectable = new CollectableObject();
	mCollectable->setEntity(ent);
	mCollectable->setObject((void *)weapon);
...
void Test::handleMouseRaycast(void)
{
	static GameUnit *lastUnitSelected = 0;

	static c_p::CollisionResult cr;
	static GameUnit *pu = 0;
	...
	mLevelManager.getCollisionManager()->getCollisionObjects(
			core::Point(v.x, v.z), c_p::COL_FLAG_UNIT_PLAYER | c_p::COL_FLAG_UNIT_ZOMBIE ,cr);
	if(!cr.empty()){
		// get the player
		 pu = static_cast<GameUnit *>(cr.front()->userDefined);

		 // if we press over the object then we select it
		if(GLOBAL_MOUSE->getMouseState().buttonDown(OIS::MB_Left)){
		 ...
		 // if the player is the same that we have last track then we do nothing
		 if(lastUnitSelected == pu) return;

		 // they are different, advise the first unit we are not mousing over it
		 // any more
		 if(lastUnitSelected) lastUnitSelected->mouseExitObject();

		 // advise the new one
		 lastUnitSelected = pu;
		 lastUnitSelected->mouseOverObject();
	} else {
		// the mouse is not any more in the object
		if(lastUnitSelected){
			lastUnitSelected->mouseExitObject();
			lastUnitSelected = 0;
		}
