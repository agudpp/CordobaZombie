...
bool Test::performMouseRay()
{
	RaycastManager *rm = GLOBAL_LVL_MNGR->getRaycastManger();
	Ogre::RaySceneQueryResult &r = rm->performOgreRay(
			mMouseCursor.getXRelativePos(),
	...
	if(r.empty()){
		debugYELLOW("EMPTY raycast\n");
	}
	ASSERT(r.size() == 1);
	Ogre::RaySceneQueryResultEntry &entry = r.back();
	ASSERT(entry.movable);
	const uint32 flags = entry.movable->getQueryFlags();
	switch(flags){
	case c_p::COL_FLAG_UNIT_CIVIL:
		debugYELLOW("Raycast with a CIVIL\n");
		break;
	case c_p::COL_FLAG_UNIT_ZOMBIE:
		debugYELLOW("Raycast with a ZOMBIE\n");
		break;
	case c_p::COL_FLAG_UNIT_PLAYER:
		debugYELLOW("Raycast with a PLAYER\n");
		break;
	default:
		debugYELLOW("Unknown type raycast\n");
	}
