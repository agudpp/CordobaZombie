                ...
                const bool remDotDirs)
{
        ...
        while(!folderStack.empty()) {
                ...
                if(!getFoldersList(actualFolder, auxList, false)){
                        return false;
                }
        ...
        if (remDotDirs){
            removeDotDirs(rootFolder, result);
        }
                              ...
                              const bool remDotDirs)
{
        ...
        if (remDotDirs){
            removeDotDirs(folderPath, result);
        }
...
//Removes first ocurrences of (basePath + "/.") and (basePath + "/..")
void FileManager::removeDotDirs(const std::string &basePath, 
                   std::list<std::string> &filePaths)
{
    std::string eraseableDOT = basePath + "/.";
    std::string eraseableDOTDOT = basePath + "/..";
       
    std::list<std::string>::iterator it;
    it = std::find(filePaths.begin(), filePaths.end(), eraseableDOT);
    if (it != filePaths.end()){
        filePaths.erase(it);
    }
    it = std::find(filePaths.begin(), filePaths.end(), eraseableDOTDOT);
    if (it != filePaths.end()){
        filePaths.erase(it);
    }    
}
