...
void Test::createSoundEmitter(void){
	Ogre::Entity *ent = GLOBAL_SCN_MNGR->createEntity("9mm.mesh");
	mSoundEmitter =
			GLOBAL_SCN_MNGR->getRootSceneNode()->createChildSceneNode(Ogre::Vector3(1024,0,1024));
}

void Test::testFollowSound(void){

	for(int i = mZombies.size()-1; i>= 0; --i){
		mZombies[i]->followSound(mSoundEmitter);
	}
}
...
//			return;
	...
	}else if(GLOBAL_KEYBOARD->isKeyDown(OIS::KC_F)){
		if (!keyPres) {
			keyPres = true;
			testFollowSound();
		}
