...
public:
    ...
    void
    updatePos(core::Vector2 &p, c_p::CollisionResult &cr)
    {
        core::Vector2 aux, pos;
        p = transVec;
...
//			aux *= len;
        ...
        if (nextX <= 0.0f || nextY <= 0.0f || nextX >= 22800.0f
            || nextY >= 22800.0f) {
            p.x = 0;
            p.y = 0;
        }
...
private:
    ...
    c_p::CollisionResult mCollResult;
